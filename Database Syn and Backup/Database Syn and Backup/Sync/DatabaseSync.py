...
def backupCurrentData(currentBackupFolderPath, tableName, cur, tableProps):
	...
	currentBackupFilePath = getFilePath(currentBackupFolderPath, "Backup_", currentDate)[0]
...
sourceEnvironment = parser.get('migrationProperties','sourceEnvironment')
destinationEnvironment = parser.get('migrationProperties','destinationEnvironment') 
specificFlag = parser.get('migrationProperties','specific')
migrationType = ("Single" if specificFlag.lower() == "true" else "Multiple")

# Prompt Password if destination environment is Prod
...
if destinationEnvironment.lower() == "prod":
...
elif destinationEnvironment.lower() == sourceEnvironment.lower():
	print("Source and Destination environment cannot be the same")
	logging.info("Source and Destination environment cannot be the same. Process terminated")
else:
	...
	if migrationType == "Single":
		...
		logging.info("Migration Type: Single - Initiated")
	...
	for databaseName in databaseNames:
		...
		for tableName  in tableNames:
			...
			currentBackupFolderPath = backupFolderPath + "\\" + destinationDatabaseName + "\\" + tableName
			
			if not os.path.exists(currentBackupFolderPath):
				os.makedirs(currentBackupFolderPath)
				
			currentBackupFilePath = backupCurrentData(currentBackupFolderPath, tableName, cur, tableProps)
	...
	currentReportFolderPath = reportFolderPath + "\\" + destinationDatabaseName + "\\" + tableName
	...
	reportFilePathData = getFilePath(currentReportFolderPath, "Report_" , currentDate)
	...
	currentReportPropertyFilePath= currentReportFolderPath + "\\" + "Report_" + currentDate.replace("-", "_") + "_v" +  str(reportFilePathData[1]) + ".txt"
